...
#+begin_src emacs-lisp
  ...
  (use-package org-roam :straight t :defer t
    :custom
...
#+begin_src emacs-lisp
  ...
  (when (equal (getenv "EXWM") "true")
    ...
    (when (executable-find "brightnessctl")
      (defun dotns/brightness-get-percent-str () "gets current brightness %"
             (let ((output (string-trim (shell-command-to-string "brightnessctl"))))
               (save-match-data
                 (string-match "\\([0-9]+%\\)" output)
                 (match-string 1 output))))

      (defun dotns/brightness-set-relative (newbrightness) ""
             (shell-command-to-string (format "brightnessctl set %s" newbrightness))
             (message "Brightness set to %s" (dotns/brightness-get-percent-str))))

    (when (executable-find "amixer")
      (defun dotns/volume-get () ""
             (let ((output (string-trim (shell-command-to-string "amixer get Master"))))
               (save-match-data
                 (string-match "\\([0-9]+%\\)" output)
                 (match-string 1 output))))

      (defun dotns/volume-set-relative (newvolume) ""
             (shell-command-to-string (format "amixer set Master %s" newvolume))
             (message "Volume set to %s" (dotns/volume-get)))

      (defun dotns/volume-toggle-mute () ""
             (interactive)
             (let ((output (string-trim (shell-command-to-string "amixer set Master toggle"))))
               (save-match-data
                 (string-match (rx (or "[on]" "[off]")) output)
                 (message "Sound %s" (match-string 0 output))))))

    (when (executable-find "flameshot")
      (defun dotns/screenshot () ""
             (interactive)
             (dotns/run-silently "flameshot gui")))

    (when (executable-find "xsecurelock")
      (defun dotns/lock-session () ""
             (interactive)
             (dotns/run-silently "xsecurelock")))

    (when (executable-find "playerctl")
      (defun dotns/media-play-pause () ""
             (interactive)
             (message "%s" (string-trim (shell-command-to-string "playerctl play-pause"))))

      (defun dotns/media-track-prev () ""
             (interactive)
             (message "%s" (string-trim (shell-command-to-string "playerctl previous"))))

      (defun dotns/media-track-next () ""
             (interactive)
             (message "%s" (string-trim (shell-command-to-string "playerctl next"))))

      (defun dotns/media-track-stop () ""
             (interactive)
             (message "%s" (string-trim (shell-command-to-string "playerctl stop")))))
      ...
      (exwm-input-global-keys
       `(([?\s-r] . exwm-reset)
         ...
         (,(kbd "<XF86MonBrightnessUp>") . (lambda ()
                                             (interactive)
                                             (when (executable-find "brightnessctl")
                                               (dotns/brightness-set-relative "10%+"))))
         (,(kbd "<XF86MonBrightnessDown>") . (lambda ()
                                             (interactive)
                                             (when (executable-find "brightnessctl")
                                               (dotns/brightness-set-relative "10%-"))))
         (,(kbd "S-<XF86MonBrightnessUp>") . (lambda ()
                                               (interactive)
                                               (when (executable-find "brightnessctl")
                                                 (dotns/brightness-set-relative "1%+"))))
         (,(kbd "S-<XF86MonBrightnessDown>") . (lambda ()
                                                 (interactive)
                                                 (when (executable-find "brightnessctl")
                                                   (dotns/brightness-set-relative "1%-"))))
         ...
         (,(kbd "S-<XF86AudioRaiseVolume>") . (lambda ()
                                               (interactive)
                                               (when (executable-find "amixer")
                                                 (dotns/volume-set-relative "1%+"))))
         (,(kbd "S-<XF86AudioLowerVolume>") . (lambda ()
                                                 (interactive)
                                                 (when (executable-find "amixer")
                                                   (dotns/volume-set-relative "1%-"))))
         (,(kbd "<XF86AudioMute>") . dotns/volume-toggle-mute)
         (,(kbd "<XF86AudioPlay>") . dotns/media-play-pause)
         (,(kbd "<XF86AudioPause>") . dotns/media-play-pause)
         (,(kbd "<XF86AudioPrev>") . dotns/media-track-prev)
         (,(kbd "<XF86AudioNext>") . dotns/media-track-next)
         (,(kbd "<XF86AudioStop>") . dotns/media-track-stop)

         (,(kbd "<print>") . dotns/screenshot)
         (,(kbd "s-l") . dotns/lock-session)
         (,(kbd "<XF86ScreenSaver>") . dotns/lock-session)
         (,(kbd "<XF86Sleep>") . dotns/lock-session)

         (,(kbd "<XF86Mail>") . (lambda () (interactive) (dotns/run-silently-split-on-prefix "terminology")))
         (,(kbd "<XF86HomePage>") . (lambda () (interactive) (dotns/run-silently-split-on-prefix "firefox")))
         (,(kbd "<XF86Explorer>") . (lambda () (interactive) (dired ".")))
